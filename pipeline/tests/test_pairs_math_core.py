"""Comprehensive tests for src.pairs.math_core."""
...
class TestComputeInteractionTerms:
    ...
    def rate_cot_only(self) -> None:
        result = compute_interaction_terms(2.0, 1.0, None, None, None)
        assert result["rate_cot"] == pytest.approx(0.3)
        assert result["vol_oi"] == 0.0
        assert result["special_rate"] == 0.0

    def vol_oi_only(self) -> None:
        result = compute_interaction_terms(None, None, 2.0, 3.0, None)
        assert result["rate_cot"] == 0.0
        assert result["vol_oi"] == pytest.approx(0.6)
        assert result["special_rate"] == 0.0
...
class TestApplyRegimeAdjustment:
    ...
    def test_trending_boosts_rate(self) -> None:
        base = {"rate": 0.5, "cot": 0.3, "vol": 0.2}
        adjusted = apply_regime_adjustment(base, "TRENDING", "NEUTRAL")
        assert adjusted["rate"] > 0.5

    def test_carry_boosts_rate_and_special(self) -> None:
        base = {"rate": 0.4, "special": 0.2, "vol": 0.4}
        adjusted = apply_regime_adjustment(base, "NEUTRAL", "CARRY")
        assert adjusted["rate"] > 0.4
        assert adjusted["special"] > 0.2

    def test_ranging_boosts_cot(self) -> None:
        base = {"rate": 0.4, "cot": 0.3, "vol": 0.3}
        adjusted = apply_regime_adjustment(base, "RANGING", "NEUTRAL")
        assert adjusted["cot"] > 0.3
...
class TestKellyFraction:
    ...
    def test_zero_avg_win_returns_zero(self) -> None:
        assert kelly_fraction(0.6, 0.0, 20.0) == 0.0
    ...
    def test_safety_factor_reduces_size(self) -> None:
        # Use values that produce positive Kelly so safety factor is visible
        full = kelly_fraction(0.60, 50.0, 20.0, safety_factor=1.0, max_risk=1.0)
        quarter = kelly_fraction(0.60, 50.0, 20.0, safety_factor=0.25, max_risk=1.0)
        assert quarter < full
        assert full > 0.0
        assert quarter > 0.0
...
class TestPairSpecificThresholds:
    @pytest.mark.parametrize(
        "pair,expected_conviction,expected_adr_mult",
        [
            ("EURUSD", 3, 1.3),
            ("USDJPY", 3, 1.5),
            ("USDINR", 4, 1.2),
    ...
    def test_known_pairs(
        self, pair: str, expected_conviction: int, expected_adr_mult: float
    ) -> None:
        thresholds = pair_specific_thresholds(pair)
        assert thresholds["conviction_enter_min"] == expected_conviction
        assert thresholds["adr_multiplier"] == expected_adr_mult
    ...
    def test_lowercase_normalized(self) -> None:
        assert pair_specific_thresholds("eurusd") == pair_specific_thresholds("EURUSD")
...
class TestCorrelationAdjustedSize:
    ...
    def test_low_corr_unchanged(self) -> None:
        portfolio = {"USDJPY": 0.3}
        corr = {"EURUSD": {"USDJPY": 0.5}}
        result = correlation_adjusted_size(1.0, "EURUSD", portfolio, corr)
        # exposure = 0.5 * 0.3 = 0.15 <= 0.3
        assert result == pytest.approx(1.0)
    ...
    def test_multiple_positions(self) -> None:
        portfolio = {"USDJPY": 0.5, "GBPUSD": 0.5}
        corr = {"EURUSD": {"USDJPY": 0.8, "GBPUSD": 0.9}}
        result = correlation_adjusted_size(1.0, "EURUSD", portfolio, corr)
        # exposure = 0.8*0.5 + 0.9*0.5 = 0.85 > 0.5
        assert result == pytest.approx(0.70)
...
class TestComputeExpectedValue:
    ...
    def test_high_conviction_boosts_ev(self) -> None:
        ev_low = compute_expected_value(0.5, 20.0, 10.0, 1, "NEUTRAL")
        ev_high = compute_expected_value(0.5, 20.0, 10.0, 5, "NEUTRAL")
...
class TestClipComposite:
    def test_in_bounds_scalar(self) -> None:
        ...
        assert clip_composite(-1.5) == pytest.approx(-1.5)
        assert clip_composite(0.0) == pytest.approx(0.0)

    def test_out_of_bounds_scalar(self) -> None:
        assert clip_composite(3.0) == pytest.approx(2.0)
        assert clip_composite(-3.0) == pytest.approx(-2.0)
        assert clip_composite(10.0) == pytest.approx(2.0)
