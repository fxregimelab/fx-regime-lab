"""EURUSD pair-specific data fetcher and signal pipeline."""
...
class EURUSDFetcher(PairPipeline):
    """EURUSD data fetcher with ECB balance sheet, EU HY OAS, and Bund-BTP proxy."""
    ...
    def _fetch_spot(self) -> float | None:
        ...
        if bars:
            return bars[-1].close
        return _retry_with_backoff(_yf_latest, "EURUSD=X", f"{self.lookback_days}d")
    ...
    def _fetch_cot(self) -> dict[str, Any]:
        try:
            ...
            logger.warning("COT fetch failed for EURUSD: %s", exc)
            return {"net_long": None, "open_interest": None, "percentile": None}
        pair_rows = [r for r in rows if r.pair == "EURUSD"]
        if not pair_rows:
            return {"net_long": None, "open_interest": None, "percentile": None}
        latest = pair_rows[-1]
    ...
    def _fetch_ecb_balance_sheet(self) -> float | None:
        """ECB balance sheet (weekly, millions of EUR). FRED: WBSDTLEZ."""
        return _retry_with_backoff(_fred_latest, "WBSDTLEZ")

    def _fetch_eu_hy_oas(self) -> float | None:
        """EU high-yield OAS (daily, basis points). FRED: BAMLEMHPAOASIOBE."""
        return _retry_with_backoff(_fred_latest, "BAMLEMHPAOASIOBE")

    def _fetch_bund_btp_proxy(self) -> dict[str, float | None]:
        """Bund-BTP spread proxy. FRED: DE 10Y + IT 10Y."""
        de_10y = _retry_with_backoff(_fred_latest, "IRLTLT01DEM156N")
        it_10y = _retry_with_backoff(_fred_latest, "IRLTLT01ITM156N")
        spread = None
        if de_10y is not None and it_10y is not None:
            spread = it_10y - de_10y
        return {"de_10y": de_10y, "it_10y": it_10y, "spread": spread}
    ...
    def fetch_all(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ecb_balance_sheet": self._fetch_ecb_balance_sheet(),
            "eu_hy_oas": self._fetch_eu_hy_oas(),
            "bund_btp": self._fetch_bund_btp_proxy(),
        }
    ...
    def compute_signals(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        bund_btp = data.get("bund_btp") or {}

        us_10y = yields.get("us_10y")
        de_10y = yields.get("de_10y")
        rate_diff = None
        if us_10y is not None and de_10y is not None:
            rate_diff = us_10y - de_10y
        ...
        if spot_bars:
            ...
            # Override vol signal with realized vol if available
        ...
        eu_hy = data.get("eu_hy_oas")
        special_val = 0.0
        special_label = "NEUTRAL"
        if eu_hy is not None:
            if eu_hy > 500:
                special_val = -0.5
                special_label = "EU_CREDIT_STRESS"
            elif eu_hy < 300:
                special_val = 0.3
                special_label = "EU_CREDIT_CALM"

        btp_spread = bund_btp.get("spread")
        if btp_spread is not None and btp_spread > 200:
            special_val -= 0.3
            special_label = f"{special_label}_FRAGMENTATION"

        stress = "HIGH" if vix is not None and vix > 30 else "NORMAL"

        score = 0.0
        if rate_signal == "BULLISH_EUR":
            score += 1.0
        elif rate_signal == "BEARISH_EUR":
            score -= 1.0
        ...
        primary = (
            "SPECIAL"
            if "STRESS" in special_label or "FRAGMENTATION" in special_label
            else "VOLATILITY"
    ...
    def compute_composite(self, signals: dict[str, Any]) -> float:
        ...
        score = 0.0
        if rate_diff is not None:
            score += max(-1.0, min(1.0, rate_diff / 2.0)) * 0.35
        if cot_pct is not None:
            score += ((cot_pct - 50) / 50) * 0.25
        score += special * 0.25
        if vix is not None:
