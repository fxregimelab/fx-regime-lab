"""EURUSD execution sizing: Kelly-optimal position sizing with pair-specific ADR.

- Kelly fraction with safety factor and max risk cap
- Pair-specific ADR multiplier (1.3× for EURUSD)
...
def compute_stop_level(
    ...
    atr: float | None,
    *,
    adr_multiplier: float = 1.3,
    mie_multiplier: float = 1.0,
) -> tuple[float | None, float | None]:
    """Return (stop_buffer, stop_price) for EURUSD.
...
def compute_eurusd_position_size(
    *,
    base_size: float,
    win_rate: float,
    avg_win_bps: float,
    ...
    """Full EURUSD position sizing pipeline.

    1. Kelly fraction
    2. Correlation adjustment vs existing positions
    3. Return final size
    """
    ...
    size = correlation_adjusted_size(size, "EURUSD", portfolio, corr_matrix)
    return size


def get_eurusd_execution_params(
    spot: float | None,
    bias: Layer2DirectionalBias,
    spot_bars: Sequence[SpotBar],
) -> dict[str, float | None]:
    """Convenience bundle: ADR, MIE, ATR, and stop levels for EURUSD."""
    thresholds = pair_specific_thresholds("EURUSD")
    adr_mult = float(thresholds.get("adr_multiplier", 1.3))
