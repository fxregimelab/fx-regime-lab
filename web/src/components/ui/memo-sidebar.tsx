...
/** Vertical memo feed + reader overlay (terminal [M] rail). */
export function MemoSidebar() {
  const listQ = useResearchMemosList();
  const [readerId, setReaderId] = useState<string | null>(null);
  const readerQ = useResearchMemoReader(readerId);

  const memos = listQ.data ?? [];

  return (
    <>
      <div className="flex h-full min-h-[60vh] w-full max-w-xl flex-col border-[var(--color-border)] bg-[var(--color-void)] text-[var(--color-text)]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
          ...
          {memos.length === 0 && !listQ.isLoading && !listQ.isError && (
            <p className="py-4 font-mono text-[10px] tracking-wider text-[var(--color-text-muted)]">
