...
export function CommandPalette() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: reset selected index on query change
  useEffect(() => {
    setSelectedIndex(0);
  }, [query]);
              ...
              {filtered.map((item, i) => (
                ...
                >
                  ...
                  {isTeleporting && teleportPath === item.path ? (
                    <span
                      className={`mt-1 block font-mono text-[9px] tabular-nums ${i === clampedIndex ? "text-[#333]" : "text-[#555]"}`}
                    >
                      <BinaryResolve
                        value={item.path}
