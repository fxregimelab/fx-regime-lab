...
export const MOCK_REGIME_CALLS: Record<string, RegimeCall> = {
  EURUSD: {
    ...
    confidence: 0.72,
    signal_composite: -0.81,
    rate_signal: 'BEARISH',
    primary_driver: 'Rate differential compressing; COT short-cover underway',
  ...
  USDJPY: {
    ...
    confidence: 0.84,
    signal_composite: 1.43,
    rate_signal: 'BULLISH',
    primary_driver: 'BoJ YCC intact; rate spread at cycle high',
    created_at: `${TODAY}T07:12:34Z`,
  },
  USDINR: {
    pair: 'USDINR',
    ...
    confidence: 0.63,
    signal_composite: 0.61,
    rate_signal: 'BULLISH',
    primary_driver: 'RBI intervention cap; DXY correlation elevated',
    created_at: `${TODAY}T07:12:34Z`,
  },
};
...
export const MOCK_SIGNALS: Record<string, SignalRow> = {
  EURUSD: {
    ...
    rate_diff_2y: -0.45,
    cot_percentile: 34,
    realized_vol_20d: 6.21,
    realized_vol_5d: 5.88,
    implied_vol_30d: 7.14,
    spot: 1.0731,
    day_change: -0.0021,
    day_change_pct: -0.2,
    created_at: `${TODAY}T07:12:34Z`,
  ...
  USDJPY: {
    ...
    rate_diff_2y: 3.82,
    cot_percentile: 78,
    realized_vol_20d: 8.13,
    realized_vol_5d: 7.94,
    implied_vol_30d: 9.32,
    spot: 154.62,
    day_change: 0.84,
    day_change_pct: 0.55,
    created_at: `${TODAY}T07:12:34Z`,
  ...
  USDINR: {
    ...
    rate_diff_2y: 4.1,
    cot_percentile: 52,
    realized_vol_20d: 3.07,
    realized_vol_5d: 3.21,
...
export const MOCK_HISTORY: Record<string, HistoryRow[]> = {
  ...
  USDINR: [
    ...
    { date: '2026-04-25', regime: 'MODERATE DEPRECIATION PRESSURE', confidence: 0.6 },
...
export const MOCK_VALIDATION: ValidationRow[] = [
  ...
  },
  {
    date: '2026-04-25',
    pair: 'USD/INR',
    call: 'MODERATE DEPRECIATION PRESSURE',
    outcome: 'incorrect',
    return_pct: -0.07,
  },
...
export const MOCK_EQUITY = {
  ...
  EURUSD: [0, 0.18, 0.04, -0.14, 0.04, 0.22, 0.48, 0.3, -0.18, -0.14, 0.04, 0.35, 0.66, 0.97],
  USDJPY: [0, 0.31, 0.55, 0.48, 0.62, 0.87, 0.61, 0.55, 0.72, 1.11, 1.5, 2.02, 2.54, 3.02],
  USDINR: [0, 0.04, 0.04, 0.04, 0.04, 0.12, 0.12, 0.02, 0.02, 0.14, 0.16, 0.28, 0.21, 0.21],
  ALL: [] as number[],
};
MOCK_EQUITY.ALL = MOCK_EQUITY.dates.map(
  (_, i) => (MOCK_EQUITY.EURUSD[i] + MOCK_EQUITY.USDJPY[i] + MOCK_EQUITY.USDINR[i]) / 3
);
